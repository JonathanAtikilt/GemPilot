...
def github_settings() -> Settings:
    return Settings(
        ...
        github_oauth_redirect_uri="http://127.0.0.1:3001/github/callback",
...
def test_github_connect_redirects_to_github_with_backend_state():
    ...
    assert query["redirect_uri"] == ["http://127.0.0.1:3001/github/callback"]
...
def test_github_connect_rejects_untrusted_return_to():
    ...
    assert response.json()["detail"] == "Invalid GitHub OAuth return URL"
...
def test_github_callback_requires_known_state():
    ...
    with TestClient(app) as client:
        response = client.get(
            "/github/callback?code=raw-oauth-code&state=wrong",
            follow_redirects=False,
        )

    assert response.status_code == 400
    assert response.json()["detail"] == "Invalid GitHub OAuth state"
    assert "raw-oauth-code" not in response.text
