...
def build_file_manifest_prompt(
    *,
    ...
    return (
        ...
        "production-style files locally. "
        "Based on the project plan and target platform, define the minimal file structure that "
        "the project needs. "
        "For web apps this will include frontend and backend directories. "
        "For CLI tools this will be a Python package. "
        "For browser extensions this will be a Manifest V3 structure. "
        "Do not include files that are not needed for this specific project type. "
        "Derive all file paths from the architecture plan and resolved tech stack — "
        "do not assume a fixed React+FastAPI layout unless the plan calls for it."
    )
...
def build_frontend_generation_prompt(
    *,
    ...
    auth_required = bool(product_brief.get("auth_required"))
    target_platform = str(product_brief.get("target_platform") or "web app")
    auth_pages = (
        ...
        if auth_required
        else "- Do NOT generate login/register/auth pages unless auth_required is true in the product brief.\n"
    )

    return (
        "You are the Frontend Generator Agent for an adaptive project generator.\n"
        "Generate frontend source files for THIS specific product — not a generic starter app.\n\n"
        f"{_code_quality_rules()}\n"
        f"Product idea:\n{idea}\n\n"
        f"Product brief:\n{_json_block(product_brief)}\n\n"
        ...
        "Requirements:\n"
        "- src/App.jsx MUST embed the product idea text and at least two idea-specific keywords.\n"
        "- Expose real product flows from user_flows as routes, tabs, or dedicated pages.\n"
        "- Wire API calls to the exact backend routes listed above — no invented study/upload/quiz routes.\n"
        "- Use feature names and user-flow screens in headings, buttons, and page titles.\n\n"
        "Generate these files with FULL CONTENT:\n"
        "- index.html — HTML shell with root div, viewport meta, and correct script src\n"
        "- package.json — dependencies and scripts for the chosen frontend stack\n"
        "- src/main.jsx — React entrypoint\n"
        "- src/App.jsx — primary product UI with tabbed or routed flows for each user journey\n"
        "- src/styles/globals.css — application styling\n"
        "- src/lib/api.js — fetch/axios helpers for EVERY backend route in the brief\n"
        f"{auth_pages}"
        "- src/pages/Dashboard.jsx — main workspace for the product's primary workflow\n"
        f"- src/pages/<Feature>Page.jsx — one page per core feature: {', '.join(features[:5])}\n"
        "- src/components/Layout.jsx — navigation shell for the product\n\n"
        "Each screen must reflect the submitted idea. No placeholder lorem ipsum or unrelated study-app flows.\n"
